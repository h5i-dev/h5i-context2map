//! The `.c2m/` workspace: cache-aware build orchestration tying the pure
//! core to persistent handles.

use crate::cache::ParseCache;
use crate::handles::{HandleRegistry, Kind};
use anyhow::{Context, Result};
use c2m_core::analysis::{assemble, Analysis};
use c2m_core::graph::{aggregate_edges, EdgeKind};
use c2m_core::ingest::{ingest, IngestOptions};
use c2m_core::types::{FileId, FileInfo, ParsedFile};
use rayon::prelude::*;
...
impl Workspace {
    ...
    pub fn build(&self, query: &str, now_epoch: u64, use_history: bool) -> Result<Built> {
        ...
        let history = if use_history {
            c2m_core::history::collect(&self.root, now_epoch)
        } else {
            c2m_core::history::History::default()
        };
