//! `c2m paint` — render *any* text (prompts, docs, tool output, files) into
...
pub fn banner(pages: usize, reflowed: bool) -> String {
    let mut b = format!(
        "c2m paint (this user's local tool) rendered the following text as {pages} image page(s) to reduce token cost. Read the pages in order; they are the referenced document content."
    );
    if reflowed {
